...
/**
* Get Current Screen for tabs.
*
* The function renders the current tab screen.
*
* @since      1.0.0
*
* @param evt       var           Click Event.
* @param wp_tab    var           Tab selected ID.
*/
function opentab( evt, wp_tab) {
	let tabcontent, tablinks;
	evt.preventDefault();
	tabcontent = jQuery( ".mwb-tabcontent" );
	tablinks = jQuery( ".mwb-tab-link" );
	
	tabcontent.hide();
	tablinks.removeClass( 'active' );
	evt.currentTarget.className += " active";
	jQuery( '#' + wp_tab ).show();
	window.location.hash = wp_tab;

	
	if( 'mwb-logs' == wp_tab && false == initiliased ) {
		initiliased = true;
		enableDataTable();
	}
	jQuery('.mwb-dashboard__header-button-wrap').css('display','flex');
	window.scrollTo(0, 0);
}
...
/**
* Get Current Screen for Default tabs.
*
* The function renders the current tab screen.
*
* @since      1.0.0
...
function opentabdefault() {
	...
	wp_tab_hash = window.location.hash;

	if (0 == wp_tab_hash.length) {
		wp_tab_hash = 'mwb-pointsandreward';
	} else {
		wp_tab_hash = wp_tab_hash.replace('#', '');
	}
	...
	jQuery('.' + wp_tab_hash + '-tab-link').addClass('active');
	jQuery('#' + wp_tab_hash).show();
}
...
jQuery( document ).ready( function($){
	...
	const trigger_error_message = ajax_data.trigger_error_message;
	const process_complete_message = ajax_data.process_complete_message;

	// Open default tab.
	opentabdefault( 'mwb-pointsandreward' );

	const triggerError = (message) => {
		swal({
			title: trigger_error_message,
			text: message,
			icon : "error",
			button: ajax_data.sweetalert_button_text
		}).then(function(){ 
			location.reload();
		});
	...
	const triggerSuccess = (message) => {
		swal({
			title: process_complete_message,
			text: message,
			icon: "success",
			button: ajax_data.sweetalert_button_text
		}).then(function(){ 
	...
	$('#mwb-pointsandreward-setting-button').on('click',function(e) {
		e.preventDefault();
		var button = $(this);
		button.addClass('mwb-ie-btn__loader');
		var enable_app      = $('#mwb_enable_pointsandreward_app').prop('checked') ? 'yes' : 'no';
		var api_key         = $('#mwb_app_pointsandreward_api_key').val();
		var login_url       = $('#mwb_app_pointsandreward_store_login_url').val();
		var register_url    = $('#mwb_app_pointsandreward_store_register_url').val();
		var widget_language = $('#mwb_app_pointsandreward_widget_language').val();
		var settings        = {enable_app, api_key, login_url, register_url, widget_language};
		var event           = 'save_pointsandreward_app_setting';
		$.post( ajax_url, { action, event, nonce, settings} ).done(function(response){
			response = JSON.parse(response);
			button.removeClass('mwb-ie-btn__loader');
			if(response.success) {
	...
	$('#mwb-pointsandreward-authorize-button').on('click',function(e) {
		e.preventDefault();
		var button = $(this);
		button.addClass('mwb-ie-btn__loader');
		var api_key = $('#mwb_app_pointsandreward_api_key').val();
		var event   = 'authorize_pointsandreward_api_key';
		$.post( ajax_url, { action, event, nonce, api_key} ).done(function(response){
			response = JSON.parse(response);
			button.removeClass('mwb-ie-btn__loader');
			if(response.success) {
				triggerSuccess(response.message);
	...
	$('#mwb_app_pointsandreward_api_key').on('click',function() {
		$(this).toggleClass("fa-eye fa-eye-slash");
		var input = $("#mwb_app_pointsandreward_api_key");
